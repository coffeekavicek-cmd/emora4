...
function publicProject(p){
  return { id:p.id, recipientName:p.recipientName, plan:p.plan, status:p.status, story:p.story, brief:p.brief||{}, media:p.media||[], slug:p.slug, customSubdomain:p.customSubdomain, publishedAt:p.publishedAt, brand:p.brand||'EMORA' };
}
...
async function handleApi(req,res,url){
  ...
  if(action==='payments/demo' && req.method==='POST'){
    if(!DEMO_MODE) return err(res,403,'Demo payment disabled');
    const b=await parseJson(req); const stage=b.stage||paidStageForStart(project.plan); const r=paymentRequirement(project,stage);
    const provider=String(b.provider||'CLICK').toUpperCase(); if(!['CLICK','PAYME'].includes(provider)) return err(res,400,'Demo provider must be CLICK or PAYME');
    const transactionId=`demo_${provider.toLowerCase()}_${crypto.randomBytes(6).toString('hex')}`;
    await mutate(db=>{const p=db.projects.find(x=>x.id===projectId); p.payments ||= []; if(!p.payments.some(x=>x.stage===stage&&x.status==='PAID')) p.payments.push({id:id('pay'),stage,status:'PAID',method:`${provider}_DEMO`,provider,transactionId,amountUzs:r.uzs,paidAt:new Date().toISOString()}); p.status=(stage==='final')?'READY_TO_FINISH':'BRIEF_READY'; p.updatedAt=new Date().toISOString();});
    return ok(res,{paid:true,provider,transactionId,requirement:r,project:await getProject(projectId)});
  }
